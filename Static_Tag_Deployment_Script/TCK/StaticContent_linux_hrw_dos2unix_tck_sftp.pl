...
$TarballArchive = '/opt/exports/TarBallsArchive';
$LogsArchive = '/opt/exports/SVN_Static_Export';
				...
				if (-d $LST)
					{
							...
							$DLST = "$LST"."/$my_file";
							if ($my_file =~/\.html$/ || $my_file =~/\.xml$/ || $my_file =~/\.js$/ || $my_file =~/\.txt$/ || $my_file =~/\.php$/|| $my_file =~/\.sh$/|| $my_file =~/\.css$/)
                                                	{
...
#system(@args) == 0 or die "system @args failed: $!";
@args = ("sh tck6_sftp.sh $Ftpfileloc");
system(@args) == 0 or die "system @args failed: $!";
